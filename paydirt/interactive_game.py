...
from .utils import (
    ...
    format_field_position_with_team,
    format_dice_roll,
    format_time,
    clamp_ball_position,
    yards_to_goal,
...
def display_game_status(game: PaydirtGameEngine, human_team: TeamChart, is_human_offense: bool):
    ...
    if COMPACT_MODE:
        ...
        print(
            f"  Timeouts: {off_team} {state.offense_timeouts} | {def_team} {state.defense_timeouts}"
        )
        print()
...
def display_box_score(game: PaydirtGameEngine, title: str = "BOX SCORE"):
    ...
    print(
        f"|  {'TEAM':<{name_width}}  |  {'1Q':>4}  {'2Q':>4}  {'3Q':>4}  {'4Q':>4}  |  {'TOTAL':>5}  |"
    )
    print("|" + "-" * 58 + "|")
    print(
        f"|  {away_name:<{name_width}}  |  {away_q[1]:>4}  {away_q[2]:>4}  {away_q[3]:>4}  {away_q[4]:>4}  |  {state.away_score:>5}  |"
    )
    print(
        f"|  {home_name:<{name_width}}  |  {home_q[1]:>4}  {home_q[2]:>4}  {home_q[3]:>4}  {home_q[4]:>4}  |  {state.home_score:>5}  |"
    )
    print("+" + "-" * 58 + "+")
                    ...
                    combined_plays.append(
                        {
                            ...
                            "pat_desc": "Extra point is good."
                            if next_play.play_type == "PAT"
                            else "Two-point conversion good.",
    ...
    print("|" + "-" * 58 + "|")
    print(
        f"|  {'First Downs':20}  |  {away_stats.first_downs:^12}  |  {home_stats.first_downs:^12}  |"
    )
    print(
        f"|  {'Total Yards':20}  |  {away_stats.total_yards:^12}  |  {home_stats.total_yards:^12}  |"
    )
    print(
        f"|  {'  Rushing':20}  |  {away_stats.rushing_yards:^12}  |  {home_stats.rushing_yards:^12}  |"
    )
    print(
        f"|  {'  Passing':20}  |  {away_stats.passing_yards:^12}  |  {home_stats.passing_yards:^12}  |"
    )
    ...
    print(
        f"|  {'  Interceptions':20}  |  {away_stats.interceptions_thrown:^12}  |  {home_stats.interceptions_thrown:^12}  |"
    )
    print(
        f"|  {'  Fumbles Lost':20}  |  {away_stats.fumbles_lost:^12}  |  {home_stats.fumbles_lost:^12}  |"
    )
    print(f"|  {'Penalties':20}  |  {away_stats.penalties:^12}  |  {home_stats.penalties:^12}  |")
    print(
        f"|  {'Penalty Yards':20}  |  {away_stats.penalty_yards:^12}  |  {home_stats.penalty_yards:^12}  |"
    )
    print(f"|  {'Sacks':20}  |  {away_stats.sacks:^12}  |  {home_stats.sacks:^12}  |")
    print(
        f"|  {'Sack Yards':20}  |  {away_stats.sack_yards:^12}  |  {home_stats.sack_yards:^12}  |"
    )
    print("+" + "-" * 58 + "+")


def _get_human_offense_play_compact(
    game: PaydirtGameEngine, state, no_huddle: bool, easy_helper=None
) -> tuple[PlayType, bool, bool, bool, bool, bool]:
    ...
    if state.down == 4:
        ...
        print(
            f"  OFF: 1-9,Q,H,S,K,P,F | N,T,/{z_option} | ?=help (Default={default_play}/{default_name})"
        )
    else:
        print(
            f"  OFF: 1-9,Q,H,S,K,P,F | N,T,/{z_option} | ?=help (Default={default_play}/{default_name})"
...
def get_human_offense_play(
    game: PaydirtGameEngine, no_huddle: bool = False, easy_helper=None
) -> tuple[PlayType, bool, bool, bool, bool, bool]:
    """
    ...
    if no_huddle:
        ...
        print("    [N] No Huddle         - Hurry-up offense (saves time, penalty risk)")
    print(
        f"    [T] Call Timeout      - Stop clock after this play ({state.offense_timeouts} remaining)"
    )
    print("    [/] Stats             - View current game statistics")
...
def get_kickoff_choice(
    game: PaydirtGameEngine, is_human_kicking: bool, ai: ComputerAI = None
) -> bool:
    """
...
def _show_easy_mode_helper(helper, game: PaydirtGameEngine, is_offense: bool):
    ...
    if is_offense:
        ...
        for i, s in enumerate(suggestions, 1):
            print(
                f"    {i}. {s['play']} - {s['success_rate']:.0f}% success, {s['avg_yards']:.1f} avg yards"
            )
        ...
        tip = helper.get_situation_tip(
            state.down, state.yards_to_go, state.quarter, state.time_remaining, score_diff
        )
        if tip:
            print(f"\n  Tip: {tip}")
    else:
...
def _get_human_defense_play_compact(
    game: PaydirtGameEngine, state, easy_helper=None
) -> tuple[DefenseType, bool]:
...
def get_human_defense_play(game: PaydirtGameEngine, easy_helper=None) -> tuple[DefenseType, bool]:
    ...
    while True:
        ...
        print(
            "  Invalid choice. Enter A, B, C, D, E, F, T, or / for stats (add T for timeout, e.g., 'AT')"
        )
...
def display_play_result(
    game: PaydirtGameEngine,
    outcome,
    play_type: PlayType,
    def_type: DefenseType,
    human_team: TeamChart,
    ...
    if getattr(outcome, "penalty_applied", False):
        if COMPACT_MODE:
            ...
            print(
                f"► PENALTY: {outcome.description}{first_down_marker} → {outcome.field_position_after}"
            )
        else:
            print("\n" + "=" * 70)
    ...
    if play_type == PlayType.PUNT:
        ...
        if COMPACT_MODE:
            ...
            if outcome.result:
                ...
                if return_dice > 0:
                    print(
                        f'  (P:{punt_dice}→"{punt_result_str}" | R:{return_dice}→"{return_yards_str}" | return)'
                    )
                else:
                    print(
                        f"  ({format_dice_roll(punt_dice, result=punt_result_str, style='verbose')})"
                    )
        ...
        if "Touchback" in desc:
            ...
            if return_dice > 0:
                ...
                print(
                    f'  (P:{punt_dice}→"{outcome.result.raw_result}" | R:{return_dice}→"F" | return)'
                )
    ...
    if play_type == PlayType.FIELD_GOAL:
        ...
        fg_nullified = (
            "UNTIMED DOWN" in outcome.description.upper()
            or "HALF EXTENDED" in outcome.description.upper()
        )
        ...
        if COMPACT_MODE:
            ...
            print(
                f"  ({format_dice_roll(dice_roll, result=chart_result, style='verbose')} | Needed: {distance_to_goal} yds)"
            )
            return
    ...
    combined = apply_priority_chart(
        outcome.result.raw_result,
        outcome.result.defense_modifier,
    ...
    if COMPACT_MODE:
        ...
        if txn and txn.is_complete:
            ...
            is_turnover_on_downs = (
                outcome.down_before == 4
                and not txn.first_down
                and not txn.turnover
                and not txn.touchdown
                and not txn.safety
            )
            ...
            if txn.has_event_type(EventType.INTERCEPTION):
                ...
                if ret_events:
                    ...
                    if txn.touchdown:
                        print(
                            f"  → Intercepted at the {int_pos_str}, returned {ret_yards} yards for a TOUCHDOWN!"
                        )
                    elif ret_yards > 0:
                        final_spot = int_spot + ret_yards
                        final_pos_str = format_field_position(final_spot)
                        print(
                            f"  → Intercepted at the {int_pos_str}, returned {ret_yards} yards to the {final_pos_str}"
                        )
                    elif ret_yards < 0:
                        print(
                            f"  → Intercepted at the {int_pos_str}, tackled {-ret_yards} yards behind the catch"
                        )
                    else:
                ...
                if txn.turnover:
                    ...
                    if ret_events:
                        ...
                        if txn.touchdown:
                            print(
                                f"  → Fumble at the {fumble_pos_str}, recovered and returned {ret_yards} yards for a TOUCHDOWN!"
                            )
                        elif ret_yards > 0:
                            print(
                                f"  → Fumble at the {fumble_pos_str}, recovered and returned {ret_yards} yards"
                            )
                        else:
                            # Defense recovered at the fumble spot (0 return yards)
                            print(
                                f"  → Fumble at the {fumble_pos_str}, recovered by the defense at the spot"
                            )
                    else:
                    ...
                    if txn.yards_gained > 0:
                        print(
                            f"  → Fumble at the {fumble_pos_str}, recovered by offense, advanced {txn.yards_gained} yards"
                        )
                    elif txn.yards_gained < 0:
                        print(
                            f"  → Fumble at the {fumble_pos_str}, recovered by offense, lost {-txn.yards_gained} yards"
                        )
                    else:
                        print(
                            f"  → Fumble at the {fumble_pos_str}, recovered by offense at the spot"
                        )
            ...
            if txn.has_event_type(EventType.INTERCEPTION):
                ...
                if ret_events:
                    ret_event = ret_events[0]
                    extra_info = (
                        f' | INT@{int_event.spot} | R:{ret_event.dice_roll}→"{ret_event.yards}"'
                    )
            elif txn.has_event_type(EventType.FUMBLE):
                fumble_event = txn.get_events_by_type(EventType.FUMBLE)[0]
                recovery_events = txn.get_events_by_type(EventType.FUMBLE_RECOVERY)
                if recovery_events:
                    ...
                    if txn.turnover:
                        ...
                        extra_info = (
                            f' | F@{fumble_event.spot} | R:{rec_event.dice_roll}→"{rec_result}"'
                        )
            ...
            print(
                f'  (O:{outcome.result.dice_roll}→"{outcome.result.raw_result}" | D:{def_row}→"{outcome.result.defense_modifier}" | {combined.priority.value}{extra_info}{breakaway_extra})'
            )
        ...
        is_turnover_on_downs = (
            outcome.down_before == 4
            and not outcome.first_down
            and not outcome.turnover
            and not outcome.touchdown
            and not outcome.safety
        )
        ...
        if (
            outcome.touchdown
            and special_marker not in turnover_td_markers
            and special_marker != " ★ TOUCHDOWN!"
        ):
            special_marker = " ★ TOUCHDOWN!"
        ...
        if play_type == PlayType.HAIL_MARY:
            ...
            print(
                f'  (O:{outcome.result.dice_roll}→"{outcome.result.raw_result}" | D:{def_row}→"{outcome.result.defense_modifier}" | {combined.priority.value}{extra_info}{breakaway_extra}{qt_extra})'
            )
    ...
    if outcome.result.result_type == ResultType.INCOMPLETE:
        ...
        int_return = (
            outcome.result.int_return_yards if hasattr(outcome.result, "int_return_yards") else 0
        )
        int_dice = (
            outcome.result.int_return_dice if hasattr(outcome.result, "int_return_dice") else 0
        )
        ...
        recovery_roll = (
            outcome.result.fumble_recovery_roll
            if hasattr(outcome.result, "fumble_recovery_roll")
            else 0
        )
        recovered = (
            outcome.result.fumble_recovered
            if hasattr(outcome.result, "fumble_recovered")
            else False
        )
        return_yards = (
            outcome.result.fumble_return_yards
            if hasattr(outcome.result, "fumble_return_yards")
            else 0
        )
        return_dice = (
            outcome.result.fumble_return_dice
            if hasattr(outcome.result, "fumble_return_dice")
            else 0
        )
        ...
        if recovered:
            ...
            if return_yards > 0:
                print(
                    f"  Return: {format_dice_roll(return_dice, result=str(return_yards), prefix='Ret')}"
                )
        else:
...
def handle_penalty_decision(
    game: PaydirtGameEngine, outcome, is_human_offense: bool, human_is_home: bool
):
    """
    ...
    if is_punt_penalty and hasattr(game, "_pending_punt_state"):
        ...
        def_result = penalty_choice.original_defense_result or (
            play_result.defense_modifier if play_result else ""
        )
    ...
    if offended_is_offense:
        filtered_penalties = [
            opt for opt in penalty_choice.penalty_options if opt.penalty_type in ["DEF", "PI"]
        ]
    else:
        filtered_penalties = [
            opt for opt in penalty_choice.penalty_options if opt.penalty_type == "OFF"
        ]
    ...
    if human_decides:
        ...
        if is_fg_penalty:
            ...
            if play_result.touchdown or play_new_pos >= 100:
                ...
                if yards_gained >= game.state.yards_to_go:
                    ...
                    play_outcome_str = (
                        f"{yards_gained} yards -> {ordinal(next_down)} and {next_ytg}"
                    )
                elif yards_gained == 0:
                    next_down = game.state.down + 1
                    play_outcome_str = (
                        f"No gain -> {ordinal(next_down)} and {game.state.yards_to_go}"
                    )
                else:
                    next_down = game.state.down + 1
                    next_ytg = game.state.yards_to_go - yards_gained  # yards_gained is negative
                    play_outcome_str = (
                        f"Loss of {abs(yards_gained)} -> {ordinal(next_down)} and {next_ytg}"
                    )
        ...
        for i, opt in enumerate(filtered_penalties):
            ...
            if adjusted_yards != opt.yards:
                ...
                if is_punt_penalty and "Replay punt" in opt.description:
                    display_desc = (
                        f"Replay punt from {pen_field_str} (half-distance: {adjusted_yards} yds)"
                    )
                elif is_kickoff_penalty and "Re-kick" in opt.description:
                    display_desc = (
                        f"Re-kick from {pen_field_str} (half-distance: {adjusted_yards} yds)"
                    )
                else:
            ...
            print(
                f"    [{i + 2}] Accept PENALTY: {display_desc} -> {pen_down_str} at {pen_field_str}"
            )
        ...
        while True:
            ...
            if choice == "1" or choice == "":
                ...
                penalty_desc = (
                    filtered_penalties[0].description if filtered_penalties else "penalty"
                )
                ...
                if 0 <= idx < len(filtered_penalties):
                    ...
                    if is_fg_penalty:
                        return game.apply_fg_penalty_decision(
                            outcome, accept_play=False, penalty_index=original_idx
                        )
                    elif is_punt_penalty:
                        return game.apply_punt_penalty_decision(
                            outcome, accept_penalty=True, penalty_index=original_idx
                        )
                    elif is_kickoff_penalty:
                        return game.apply_kickoff_penalty_decision(outcome, accept_penalty=True)
                    else:
                        return game.apply_penalty_decision(
                            outcome, accept_play=False, penalty_index=original_idx
                        )
                else:
        ...
        cpu_team = (
            game.state.defense_team.peripheral.short_name
            if is_human_offense
            else game.state.possession_team.peripheral.short_name
        )
...
def _offer_record_to_standings(
    year: int, home_team: str, home_score: int, away_team: str, away_score: int, week: int = 0
):
    ...
    if record == "y":
        ...
        if success:
            ...
            print(
                f'  You can manually add with: python -m paydirt.standings add {year} "{home_team}" {home_score} "{away_team}" {away_score}{week_arg}'
            )
    else:
        print("  Game not recorded.")
        week_arg = f" --week {week}" if week > 0 else ""
        print(
            f'  To record later: python -m paydirt.standings add {year} "{home_team}" {home_score} "{away_team}" {away_score}{week_arg}'
        )


def resume_game(
    save_file: str = None, difficulty: str = "medium", compact: bool = True, week: int = 0
):
    """
    ...
    if cpu_ai.use_analysis:
        loaded_opponent_model = load_ai_data(
            game.state.away_chart.team_dir, game.state.home_chart.team_dir, save_dir
        )
    ...
    if pending_score == "touchdown":
        ...
        if scoring_team_is_home:
            ...
            game.state.scoring_plays.append(
                ScoringPlay(
    ...
    while not game.state.game_over:
        ...
        if game.state.quarter != last_quarter:
            if last_quarter == 2 and game.state.quarter == 3:
                ...
                kicking_team = (
                    home_chart.peripheral.short_name
                    if second_half_kicking_home
                    else away_chart.peripheral.short_name
                )
                receiving_team = (
                    away_chart.peripheral.short_name
                    if second_half_kicking_home
                    else home_chart.peripheral.short_name
                )
                ...
                if outcome.pending_penalty_decision and outcome.penalty_choice:
                    ...
                    outcome = handle_penalty_decision(
                        game, outcome, is_human_offense, human_is_home
                    )
        ...
        # Get play calls
        if is_human_offense:
            play_type, no_huddle_mode, out_of_bounds, in_bounds, call_timeout, call_spike = (
                get_human_offense_play(game, no_huddle_mode, easy_mode_helper)
            )

            if play_type is None:
                filepath = save_game(
                    game,
                    human_is_away=not human_is_home,
        ...
        is_turnover_on_downs = (
            ...
            and not outcome.touchdown
            and not outcome.safety
            and outcome.play_type not in [PlayType.PUNT, PlayType.FIELD_GOAL]
        )
        if is_turnover_on_downs:
            print("\n  *** TURNOVER ON DOWNS ***")
            ...
            if total_plays >= 5:
                ...
                for down, dist, name in common_situations:
                    ...
                    if tendency.total_plays > 0:
                        ...
                        print(
                            f"    {name}: {tendency.run_plays}R/{tendency.pass_plays}P -> recommend {rec}"
                        )
            else:
                print(
...
def run_interactive_game(
    ...
    while not game.state.game_over:
        ...
        if time_effectively_zero and game.has_untimed_down():
            ...
            if game.state.is_overtime:
                ...
                if game.needs_overtime():
                    ...
                    print(f"  Overtime period: {rules.period_length_minutes:.0f} minutes")
                    print(
                        f"  Format: {'Sudden Death' if rules.format.value == 'sudden_death' else 'Modified Sudden Death'}"
                    )
        ...
        is_turnover_on_downs = (
            outcome.down_before == 4
            and not outcome.first_down
            and not outcome.turnover  # Not INT/fumble
            and not outcome.touchdown
            and not outcome.safety
            and outcome.play_type not in [PlayType.PUNT, PlayType.FIELD_GOAL]
        )
