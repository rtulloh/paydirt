...
from .game_engine import PaydirtGameEngine, GameState
from .play_resolver import (
    PlayType, DefenseType, PlayResult, ResultType,
    resolve_play, roll_chart_dice, is_passing_play,
...
def get_human_offense_play(game: PaydirtGameEngine, no_huddle: bool = False) -> tuple[PlayType, bool, bool, bool, bool]:
    ...
    if state.down == 4:
        ...
        # Punt option
        print(f"    [P] PUNT              - Kick the ball away")
    ...
    print("\n  OPTIONS:")
    if no_huddle:
        print(f"    [N] EXIT No Huddle    - Return to normal offense")
    else:
        print(f"    [N] No Huddle         - Hurry-up offense (saves time, penalty risk)")
    print(f"    [T] Call Timeout      - Stop clock after this play ({state.offense_timeouts} remaining)")
    print(f"    [/] Stats             - View current game statistics")
    print(f"    Add '+' for Out of Bounds (e.g., '5+' = stops clock, costs 5 yards)")
    print(f"    Add '-' for In Bounds (e.g., '5-' = keeps clock running, costs 5 yards)")
...
def display_play_result(game: PaydirtGameEngine, outcome, play_type: PlayType, 
    ...
    if outcome.result.result_type == ResultType.INCOMPLETE:
        ...
        if outcome.touchdown:
            print(f"  >>> PICK SIX! Returned for a TOUCHDOWN!")
        ...
        if recovered:
            ...
            print(f"  >>> DEFENSE RECOVERS - TURNOVER!")
            if return_yards > 0:
                print(f"  Return: {return_yards} yards (roll: {return_dice})")
        if outcome.touchdown:
            print(f"  >>> FUMBLE RETURN TOUCHDOWN!")
    ...
    if human_decides:
        ...
        while True:
            ...
            if choice == '0' or choice == '':
                ...
                penalty_desc = penalty_choice.penalty_options[0].description if penalty_choice.penalty_options else "penalty"
                print(f"\n  >> {penalty_desc.upper()} - DECLINED. Result of the play stands.")
                return game.apply_penalty_decision(outcome, accept_play=True)
            elif choice.isdigit():
                idx = int(choice) - 1
                if 0 <= idx < len(penalty_choice.penalty_options):
                    opt = penalty_choice.penalty_options[idx]
                    print(f"\n  >> Accepting penalty: {opt.description}")
                    return game.apply_penalty_decision(outcome, accept_play=False, penalty_index=idx)
                else:
        ...
        if play_td:
            ...
            best_penalty = penalty_choice.penalty_options[0] if penalty_choice.penalty_options else None
            if best_penalty:
        ...
        if accept_play:
            ...
            opt = penalty_choice.penalty_options[best_penalty_idx]
            print(f"\n  >> {cpu_team} accepts penalty: {opt.description}")
            return game.apply_penalty_decision(outcome, accept_play=False, penalty_index=best_penalty_idx)
...
def run_interactive_game(difficulty: str = 'medium'):
    ...
    human_receives_second_half = not human_receives_first
