...
const PenaltyDecisionPanel = ({ penaltyData, onDecision }) => {
  if (!penaltyData) {
    return null;
  }
  ...
  const formatFieldPosition = (pos) => {
    if (pos <= 50) return `OWN ${pos}`;
    return `OPP ${100 - pos}`;
  };

  const getPlayResultSummary = () => {
    if (turnover) {
      return `TURNOVER at ${formatFieldPosition(newBallPosition)}`;
    }
    if (touchdown) {
      return 'TOUCHDOWN!';
    }
    if (yards > 0) {
      return `+${yards} yards → ${formatFieldPosition(newBallPosition)}`;
    }
    return `No gain → ${formatFieldPosition(newBallPosition)}`;
  };
