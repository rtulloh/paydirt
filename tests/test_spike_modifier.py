...
# Methods are now on the game engine:
# game._apply_spike(time_before_play, quarter_before_play)
# game._should_apply_spike_after_play(outcome, play_seconds)
...
class TestApplySpike:
    """Tests for _apply_spike method on game engine."""
...
class TestShouldApplySpikeAfterPlay:
    """Tests for _should_apply_spike_after_play method on game engine."""
    ...
    def test_spike_not_applied_after_incomplete(self, game):
        """Spike should not be applied after incomplete pass (clock already stopped)."""
        outcome = self.create_outcome(ResultType.INCOMPLETE)
        should_apply, msg = game._should_apply_spike_after_play(outcome, play_seconds=25)

        assert should_apply is False
        assert "clock already stopped" in msg.lower()

    def test_spike_applied_after_yards(self, game):
    ...
    def test_spike_applied_after_breakaway(self, game):
        """Spike should be applied after breakaway play."""
        outcome = self.create_outcome(ResultType.BREAKAWAY)
        should_apply, msg = game._should_apply_spike_after_play(outcome, play_seconds=20)
        
        assert should_apply is True
    
    def test_spike_not_applied_when_play_was_short(self, game):
        """Spike should not be applied if play used <=3 seconds."""
        outcome = self.create_outcome(ResultType.YARDS)
        should_apply, msg = game._should_apply_spike_after_play(outcome, play_seconds=2)
...
class TestSpikeModifierIntegration:
    ...
    def test_spike_modifer_reduces_time_to_20_seconds(self, game):
        ...
        # Apply spike
        should_apply, _ = game._should_apply_spike_after_play(outcome, play_seconds=35)
        if should_apply:
            game._apply_spike(time_before_play, quarter_before_play)
    ...
    def test_spike_modifier_with_two_minute_drill(self, game):
        ...
        # Apply spike
        should_apply, _ = game._should_apply_spike_after_play(outcome, play_seconds=25)
        if should_apply:
            game._apply_spike(time_before_play, quarter_before_play)
    ...
    def test_spike_capped_play_allows_field_goal(self, game):
        ...
        # Apply spike
        should_apply, _ = game._should_apply_spike_after_play(outcome, play_seconds=45)
        if should_apply:
            game._apply_spike(time_before_play, quarter_before_play)
