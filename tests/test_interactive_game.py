...
requires_1972_season = pytest.mark.skipif(
    not Path("seasons/1972").exists(), reason="1972 season data not available"
)
requires_1983_season = pytest.mark.skipif(
    not Path("seasons/1983").exists(), reason="1983 season data not available"
)
...
class TestAIOpponentAnalysis:
    ...
    def test_opponent_tracker_records_plays(self):
        ...
        ai.opponent_model.record_opponent_play(
            down=1, distance=10, play_type="Short Pass", yards_gained=5, is_pass=True
        )
        ai.opponent_model.record_opponent_play(
            down=2, distance=7, play_type="Run", yards_gained=3, is_pass=False
        )
    ...
    def test_opponent_tracker_calculates_tendencies(self):
        ...
        for _ in range(4):
            ai.opponent_model.record_opponent_play(
                down=1, distance=10, play_type="Long Pass", yards_gained=15, is_pass=True
            )
        for _ in range(1):
            ai.opponent_model.record_opponent_play(
                down=2, distance=5, play_type="Run", yards_gained=2, is_pass=False
            )
    ...
    def test_opponent_tracker_gets_defense_recommendation(self):
        ...
        for _ in range(7):
            ai.opponent_model.record_opponent_play(
                down=3, distance=8, play_type="Long Pass", yards_gained=10, is_pass=True
            )
        for _ in range(3):
            ai.opponent_model.record_opponent_play(
                down=3, distance=8, play_type="Run", yards_gained=2, is_pass=False
            )
...
class TestSelectTeamTwoStep:
    ...
    def test_select_team_calls_select_season_first(
        self, mock_load, mock_get_teams, mock_select_season
    ):
...
class TestReturnValueSignature:
    ...
    def test_get_human_offense_play_returns_six_values(self, game):
        """Verify get_human_offense_play returns exactly 6 values."""
        from paydirt.interactive_game import get_human_offense_play
        ...
        with patch("paydirt.interactive_game.analyze_team_strength", return_value="balanced"):
            with patch("builtins.input", side_effect=["7"]):
                result = get_human_offense_play(game, no_huddle=False, easy_helper=None)

        assert len(result) == 6, f"Expected 6 return values, got {len(result)}"

    def test_get_human_offense_play_tuple_structure(self, game):
        ...
        play_type, no_huddle_for_next, out_of_bounds, in_bounds, call_timeout, call_spike = result

        assert isinstance(play_type, PlayType)
        assert isinstance(no_huddle_for_next, bool)
        assert isinstance(out_of_bounds, bool)
        assert isinstance(in_bounds, bool)
        assert isinstance(call_timeout, bool)
