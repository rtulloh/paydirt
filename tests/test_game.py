    ...
    def test_run_play_turnover(self, game):
        ...
        with patch("paydirt.game.resolve_play") as mock_resolve:
            ...
            mock_resolve.return_value = (
                2,
                PlayOutcome(PlayResult.FUMBLE, -2, "Fumble!", turnover=True),
            )
...
class TestUsePlayTime:
    """Tests for _use_play_time function."""

    def test_use_play_time_with_specific_value(self):
        """_use_play_time should reduce time."""
        ...
        game._use_play_time(10.0)

        assert game.state.time_remaining < initial_time

    def test_use_play_time_default_random(self):
        """_use_play_time with no args should use random time in valid range."""
        from paydirt.models import Team
        from paydirt.game import PaydirtGame
...
class TestAISelectPlay:
    """Tests for AI play selection functions."""

    def test_game_has_ai_methods(self):
        """Game should have AI-related methods."""
        from paydirt.models import Team
        from paydirt.game import PaydirtGame

        home = Team(name="Home", abbreviation="HOM", rushing_offense=7, passing_offense=5)
        away = Team(name="Away", abbreviation="AWY", rushing_defense=5, passing_defense=7)
        game = PaydirtGame(home, away)
        game.kickoff(receiving_team_is_home=False)

        # Verify game has run_play method (the main play execution)
        assert hasattr(game, "run_play")

        # Verify we can call kickoff to set up the game
        result = game.kickoff(receiving_team_is_home=False)
        assert result is not None


class TestAISelectDefense:
    """Tests for AI defense selection functions."""

    def test_game_has_defense_methods(self):
        """Game should have methods for handling defense."""
        from paydirt.models import Team
        from paydirt.game import PaydirtGame
        ...
        # Verify run_play accepts defense parameter
        assert hasattr(game, "run_play")
